...
export function MinimalistTemplate({ data, colorScheme }: TemplateProps) {
  const { basicInfo, strengths, workExperience, projectExperience } = data

  return (
    <div className="resume-tpl resume-tpl--minimalist" style={{ '--tpl-primary': colorScheme.primary, '--tpl-secondary': colorScheme.secondary, '--tpl-accent': colorScheme.accent } as React.CSSProperties}>
      <header className="resume-tpl__header">
        <h1 className="resume-tpl__name">{basicInfo.name}</h1>
        <div className="resume-tpl__meta">
          {[basicInfo.gender, basicInfo.age, basicInfo.phone, basicInfo.email].filter(Boolean).join(' | ')}
        </div>
        <div className="resume-tpl__meta">
          {[basicInfo.university, basicInfo.education, basicInfo.major].filter(Boolean).join(' | ')}
        </div>
        <div className="resume-tpl__meta">
